...
This raises a genuine typing question. Three states are easily collapsed in prose, but they must remain distinct:

\paragraph{Initial state $\iota$.} The accumulator has received no input. This is a process state, not a quantitative verdict.

\paragraph{Unresolved verdict $\bot$.} A reading was attempted but failed to resolve. This is a failure verdict, not a number.

\paragraph{Resolved zero $\mathrm{Resolved}(\zeroQ)$.} The reading succeeded and retained no difference. This is a determinate quantitative verdict.
...
\paragraph{Accumulation.} Within one reading, contributions are combined across the traversed edges. An initial process state is neutral for this operation.

\paragraph{Pipeline sequencing.} Across completed reading stages, an unresolved stage is absorbing: failure at one stage prevents the pipeline from reporting a resolved result.
...
This yields the practical diagnosis. The contamination is not a defect in arithmetic zero. It occurs when one untagged value is forced to represent several states: a successful reading that retained no difference, a reading that did not resolve, and an accumulator that was never initialised. Removing the numeral does not repair that representation; another boundary value can be overloaded in exactly the same way. The repair is a sum type whose constructors preserve the distinction. The difference is behavioural rather than merely terminological: \verb|pipeline_seq (Resolved 0) (Resolved q)| returns \verb|Resolved q|, while \verb|pipeline_seq Unresolved (Resolved q)| returns \verb|Unresolved|. Thus the formal result supports a criticism of type design and silent failure conversion, not a criticism of zero itself.
